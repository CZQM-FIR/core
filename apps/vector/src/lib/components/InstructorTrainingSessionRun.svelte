<script lang="ts">
	...
	let objectiveResults = $state(
		untrack(() =>
			(session.objectiveResults ?? []).map((result) => ({
				text: result.text,
	...
	const objectiveSummary = $derived.by(() => {
		if (objectiveResults.length === 0) return null;
		const count = `${achievedCount} of ${objectiveResults.length} objectives achieved`;
		if (session.notesLocked) {
			return session.taskComplete
				? `${count} — the course task was marked complete.`
				: `${count} — the course task is not yet complete.`;
		}
		if (!allObjectivesAchieved) {
			return `${count} — the course task will remain incomplete.`;
		}
		if (!session.taskComplete) {
			return `${count} — the course task will be marked complete when notes are submitted.`;
		}
		return count;
	});
...
<div class="card bg-base-200 mt-6 shadow-sm">
	<div class="card-body gap-3">
		...
		<div>
			<h3 class="text-sm font-medium">Session objectives</h3>
			{#if objectiveResults.length === 0}
				<p class="text-warning mt-2 text-sm">
					This session has no objectives configured. Submitting notes will not complete the course
					task until an admin adds them.
				</p>
			{:else}
				<ul class="mt-2 flex flex-col gap-2">
					{#each objectiveResults as result (result.text)}
						<li>
							<label class="flex items-start gap-2">
								<input
									type="checkbox"
									class="checkbox checkbox-sm mt-0.5"
									bind:checked={result.achieved}
									onchange={scheduleAutosave}
									disabled={!session.canSaveNotes}
								/>
								<span class="text-sm">{result.text}</span>
							</label>
						</li>
					{/each}
				</ul>
				<p class="mt-2 text-sm opacity-80">{objectiveSummary}</p>
			{/if}
		</div>
		<textarea
